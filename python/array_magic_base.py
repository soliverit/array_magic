...
##
	...
	def __init__(self): 
		...
		self.reportSteps	= DEFAULT_REPORT_STEPS	
		self.nSamples		= DEFAULT_N_SAMPLES
		self.trainTestRatio	= DEFAULT_TRAIN_TEST
		self.iterations		= DEFAULT_ITERATIONS
		self.maxError		= DEFAULT_MAX_ERROR
		self.nInputs		= DEFAULT_N_INPUTS
		# Set misc defaults
		self.silent			= False
	##
	# Train the machine.
	...
	def train():
		...
		##
		self.machine = self.doTraining()
	##
	# Run a test using the self.xData[:test], self.yData[:test] samples.
	#
	...
	# in self.xData{train:[], test:[]} and self.yData{train: [], test:[]} but
	...
	# properties (self.maxError, self.iterations,...) to train a model 
	...
	def predict(inputs): #Virtual
		return self.machine.run(inputs)
